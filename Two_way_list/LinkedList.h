...
public:
    ...
    [[nodiscard]]
    Node* find_last(const Vehicle& data) const {
    ...
    template<class Predicate>
    [[nodiscard]]
    Node* find_first(const Vehicle& data, Predicate predicate, Node* start = nullptr) const {
    ...
    [[nodiscard]]
    Node* find_first(const Vehicle& data, Node* start = nullptr) const {
        for (auto item = start != nullptr ? start : head; item != nullptr; item = item->next)
    ...
    [[nodiscard]]
    Node* find_ordinal(const Vehicle& data, const size_t& ordinal) const {
        Node* item = nullptr;
